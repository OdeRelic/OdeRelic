...
QVariantMap PS1XstationLibraryService::checkXStationFolder(const QString &rootPath) {
  QVariantMap result;
...
QVariantMap PS1XstationLibraryService::tryDetermineGameIdFromHex(const QString &filepath) {
  QVariantMap result;
      ...
      while (!in.atEnd()) {
        ...
        if (line.startsWith("FILE")) {
          ...
          if (firstQuote != -1 && lastQuote != -1 && lastQuote > firstQuote) {
            QString binFileName = line.mid(firstQuote + 1, lastQuote - firstQuote - 1);
  ...
  const qint64 MAX_SEARCH_BYTES = 8 * 1024 * 1024; // Enforce max 8MB PS1 bound preventing freezes
  qint64 totalRead = 0;
  QByteArray carry;
  ...
  file.close();
  result["success"] = false;
  qWarning() << "PS1 ID Scraper: Completed scraping bounds; no valid identifiers naturally found.";
  return result;
}

QVariantMap PS1XstationLibraryService::createXStationFolder(const QString &rootPath) {
  QVariantMap result;
  QString cleanPath = urlToLocalFile(rootPath);
  QDir dir(cleanPath);
  ...
  if (!dir.exists("00xstation")) {
    if (dir.mkdir("00xstation")) {
      ...
      result["success"] = false;
      result["message"] = "Failed to create 00xstation volume. Check physical locking switch.";
      return result;
    }
  } else {
    result["success"] = true;
...
void PS1XstationLibraryService::startXStationSetupAsync(const QString &rootPath, const QString &odeType) {
  std::thread([this, rootPath, odeType]() {
    QNetworkAccessManager manager;
    QEventLoop loop;
    QNetworkRequest apiRequest(QUrl("https://api.github.com/repos/x-station/xstation-releases/releases/latest"));
    apiRequest.setRawHeader("User-Agent", "OdeRelic-XStation/1.0");

    QMetaObject::invokeMethod(this, [=]() { emit setupXStationProgress(5, "Contacting XStation Github Core API..."); }, Qt::QueuedConnection);
    ...
    if (apiReply->error() != QNetworkReply::NoError) {
      QMetaObject::invokeMethod(this, [=]() { emit setupXStationFinished(false, "Failed to reach GitHub API securely."); }, Qt::QueuedConnection);
    ...
    if (downloadUrl.isEmpty()) {
      QMetaObject::invokeMethod(this, [=]() { emit setupXStationFinished(false, "Could not locate valid update.zip asset within latest release bounds."); }, Qt::QueuedConnection);
      return;
    }

    QMetaObject::invokeMethod(this, [=]() { emit setupXStationProgress(20, "Downloading native XStation firmware array..."); }, Qt::QueuedConnection);

    QString tempDir = QDir::tempPath() + "/oderelic_xstation_" + QString::number(QDateTime::currentMSecsSinceEpoch());
    QDir().mkpath(tempDir);
    ...
    if (dlReply->error() != QNetworkReply::NoError) {
      QMetaObject::invokeMethod(this, [=]() { emit setupXStationFinished(false, "Network interruption detected cleanly halting deployment natively."); }, Qt::QueuedConnection);
      dlReply->deleteLater();
      return;
    }
    ...
    QMetaObject::invokeMethod(this, [=]() { emit setupXStationProgress(60, "Extracting payload mapping via OS native zipper..."); }, Qt::QueuedConnection);
    ...
    if (unzipProcess.exitCode() != 0) {
      QMetaObject::invokeMethod(this, [=]() { emit setupXStationFinished(false, "Failed to natively extract zipper mapped package."); }, Qt::QueuedConnection);
      return;
    }

    QMetaObject::invokeMethod(this, [=]() { emit setupXStationProgress(85, "Flashing extracted structural nodes identically across SD Card boundaries..."); }, Qt::QueuedConnection);
    ...
    QMetaObject::invokeMethod(this, [=]() { emit setupXStationProgress(100, "Setup complete!"); }, Qt::QueuedConnection);
    QMetaObject::invokeMethod(this, [=]() { emit setupXStationFinished(true, "XStation PS1 OS deployment fully integrated successfully!"); }, Qt::QueuedConnection);

  }).detach();
}

void PS1XstationLibraryService::checkXStationUpdateAsync(const QString &rootPath) {
  // Empty stub
}

void PS1XstationLibraryService::scanExternalFilesAsync(const QStringList &fileUrls, bool dummy) {
  std::thread([this, fileUrls]() {
    QVariantList files = getExternalGameFilesData(fileUrls);
    QMetaObject::invokeMethod(this, [this, files]() {
      emit externalFilesScanFinished(true, files);
    }, Qt::QueuedConnection);
  }).detach();
}

static void scanDirectoryRecursivelyXs(PS1XstationLibraryService *service, const QString &dirPath, QFileInfoList &toProcess) {
  QDirIterator it(dirPath, QDir::Files | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
  while (it.hasNext()) {
    it.next();
    QString ext = it.fileInfo().suffix().toLower();
    if (ext == "iso" || ext == "bin" || ext == "cue" || ext == "img" || ext == "chd") {
      toProcess.append(it.fileInfo());
    }
  }
}

QVariantList PS1XstationLibraryService::getExternalGameFilesData(const QStringList &fileUrls) {
  QVariantList files;
  for (const QString &urlStr : fileUrls) {
    ...
    for (const QFileInfo &fi : toProcess) {
      if (fi.suffix().toLower() == "bin") {
        ...
        if (!cues.isEmpty()) continue; // Let the CUE intelligently map its tracks natively safely.
      }
      filtered.append(fi);
    }
    ...
    for (const QFileInfo &fileInfo : filtered) {
      if (fileInfo.exists() && fileInfo.isFile()) {
        ...
        qint64 rSize = (ext == "cue") ? SystemUtils::calculateCueRealSize(fileInfo) : fileInfo.size();
        itemInfo["size"] = rSize;
        itemInfo["stats"] = QVariantMap{{"size", rSize}};
        // Fake ID detection mapping
        QRegularExpression idRegex("(SLUS|SCUS|SLES|SCES|SLPM|SLPS|SCPS|SCPM|SLAJ|SCAJ|SLKA|SCKA|SCED|SCCS)[_-]?\\d{3}\\.?\\d{2}");
  ...
  std::thread([this, sourceIsoPath, targetLibraryRoot, gameId, gameName]() {
    ...
    for (const QFileInfo &f : itemsToCopy) {
      if (cancelRequested.load()) {
        QMetaObject::invokeMethod(this, [=]() { emit importIsoFinished(sourceIsoPath, false, "", "Import gracefully cancelled unconditionally mapping correctly."); }, Qt::QueuedConnection);
        return;
      }
      ...
      while ((bytesRead = srcFile.read(buffer, sizeof(buffer))) > 0) {
        if (cancelRequested.load()) {
          ...
          QMetaObject::invokeMethod(this, [=]() { emit importIsoFinished(sourceIsoPath, false, "", "Cancelled mapping effectively."); }, Qt::QueuedConnection);
          return;
        }
    ...
    if (allSuccess) {
      QMetaObject::invokeMethod(this, [=]() { emit importIsoProgress(sourceIsoPath, 100, 0.0); }, Qt::QueuedConnection);
      QMetaObject::invokeMethod(this, [=]() { emit importIsoFinished(sourceIsoPath, true, finalDestDir, "Import completed natively implicitly smoothly."); }, Qt::QueuedConnection);
    } else {
      QMetaObject::invokeMethod(this, [=]() { emit importIsoFinished(sourceIsoPath, false, "", "Native system error mapping I/O bytes sequentially."); }, Qt::QueuedConnection);
    }
  }).detach();
}

void PS1XstationLibraryService::startDownloadArtAsync(const QString &dirPath, const QString &gameId, const QString &callbackSourceKey) {
  std::thread([this, dirPath, gameId, callbackSourceKey]() {
    ...
    // XStation strictly dictates explicitly sized 80x84 Windows BMP structures statically inside each Game directory globally.
    ...
    for (const QString &type : types) {
      ...
      if (reply->error() == QNetworkReply::NoError) {
        ...
        if (srcImg.loadFromData(imageData)) {
          // Downsample and explicitly structure natively into BMP format exclusively accepted by XStation
          QImage resized = srcImg.scaled(80, 84, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    ...
    if (artFound) {
      QMetaObject::invokeMethod(this, [=]() { emit artDownloadFinished(callbackSourceKey, true, "Native 80x84 BMP converted efficiently."); }, Qt::QueuedConnection);
    } else {
      QMetaObject::invokeMethod(this, [=]() { emit artDownloadFinished(callbackSourceKey, false, "Luden02 backend block mapping missing."); }, Qt::QueuedConnection);
    }
  ...
  std::thread([this, gamesList]() {
    ...
    for (const QVariant &item : gamesList) {
      ...
      if (extractedId.isEmpty()) {
          QVariantMap hexScan = tryDetermineGameIdFromHex(path + "/" + binaryFileName);
          if (hexScan.value("success").toBool()) extractedId = hexScan.value("gameId").toString();
      }
      ...
      current++;
      QMetaObject::invokeMethod(this, [=]() { emit batchArtDownloadProgress(current, total); }, Qt::QueuedConnection);
    }
    
    QMetaObject::invokeMethod(this, [=]() { emit batchArtDownloadFinished(!cancelRequested.load()); }, Qt::QueuedConnection);
  }).detach();
}
...
void PS1XstationLibraryService::startGetGamesFilesAsync(const QString &dirPath) {
  ...
  std::thread([this, dirPath]() {
    ...
    QDirIterator it(cleanT, QDir::Dirs | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
    ...
    while (it.hasNext()) {
      ...
      if (scannedFolders % 10 == 0) {
        QMetaObject::invokeMethod(this, [=]() { emit libraryScanProgress(scannedFolders, 0); }, Qt::QueuedConnection);
      }
      
      QFileInfo dirInfo = it.fileInfo();
      if (dirInfo.fileName() == "00xstation") continue;
      ...
      if (!files.isEmpty()) {
        ...
        QFileInfoList allFiles = gameDir.entryInfoList(QDir::Files | QDir::Hidden | QDir::System);
    ...
    QMetaObject::invokeMethod(this, [=]() { emit gamesFilesLoaded(dirPath, result); }, Qt::QueuedConnection);
  }).detach();
}
