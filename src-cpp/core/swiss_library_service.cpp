...
void SwissLibraryService::cancelAllImports() { m_cancelRequested = true; }

void SwissLibraryService::resetCancelFlag() { m_cancelRequested = false; }
...
void SwissLibraryService::startGetGamesFilesAsync(const QString &dirPath) {
  std::thread([this, dirPath]() {
    ...
    for (const QString &subDir : subDirs) {
      ...
      QDirIterator it(targetDirStr, QDir::Files | QDir::NoDotAndDotDot,
      ...
      while (it.hasNext()) {
        ...
        if (ext == "iso" || ext == "gcm") {
          ...
          QString pathKey =
              fileInfo.fileName() + "_" + QString::number(actualSize);
...
QVariantMap
SwissLibraryService::tryDetermineGameIdFromHex(const QString &filepath) {
  QVariantMap result;
  ...
  result["success"] = false;
  result["message"] =
      "Could not locate a GameCube game ID inside the provided file.";
  return result;
}

bool SwissLibraryService::provisionCheat(const QString &gameId,
                                         const QString &libraryRoot) {
  if (gameId.isEmpty() || libraryRoot.isEmpty())
    return false;
  ...
  QString cheatsDir =
      QUrl::fromPercentEncoding(libraryRoot.toUtf8()) + "/cheats";
...
QVariantMap SwissLibraryService::renameGamefile(
  ...
  int discNum = 1;
  QRegularExpression discRegex("\\(Disc\\s*([1-9])\\)",
  ...
  if (dirpath.contains("/.orbit_temp_")) {
    // This is an internal temporary file created by our BIN2ISO converter, so
    // we can securely move it
    QFile file(dirpath);
    ...
    // Strict User File Preservation: ONLY copy the file, do not remove the
    // original
    if (QFile::copy(dirpath, newFilePath)) {
      provisionCheat(gameId, targetLibraryRoot);
      result["success"] = true;
...
void SwissLibraryService::startImportIsoAsync(const QString &sourceIsoPath,
  ...
  std::thread([this, sourceIsoPath, targetLibraryRoot, gameId, gameName]() {
    ...
    if (isRvz) {
      ...
      bool success =
          converter.convertRvzToIso(sourceIsoPath, destIsoPath, errMsgs);
      if (success) {
        provisionCheat(gameId, targetLibraryRoot);
        QMetaObject::invokeMethod(
            this,
            [=]() {
              emit importIsoFinished(sourceIsoPath, true, destIsoPath, errMsgs);
            },
            Qt::QueuedConnection);
      } else {
        QMetaObject::invokeMethod(
            this,
            [=]() {
              emit importIsoFinished(sourceIsoPath, false, "", errMsgs);
            },
            Qt::QueuedConnection);
      }
      return;
    ...
    // accidentally rip the file away from the user's origin folder
    ...
    if (bytesProcessed == totalBytes && totalBytes > 0) {
      // PRESERVE: Removing the QFile::remove(sourceIsoPath) block. Origin
...
void SwissLibraryService::startDownloadArtAsync(
  ...
  QThread *workerThread =
      QThread::create([this, dirPath, gameId, callbackSourceKey]() {
        QDir().mkpath(dirPath);

        QNetworkAccessManager manager;

        // We try to pull from GameTDB US region for gamecube. E.g.
        // https://art.gametdb.com/wii/cover/US/GALE01.png If not found, try EN.
        QStringList regions = {"US", "EN", "JA", "EU"};
        bool success = false;

        for (int i = 0; i < regions.size(); i++) {
          QString urlStr = "https://art.gametdb.com/wii/cover/" + regions[i] +
                           "/" + gameId + ".png";
        ...
        QMetaObject::invokeMethod(
            ...
            [=]() {
              ...
              emit artDownloadFinished(
                  callbackSourceKey, success,
                  success ? "Downloaded art successfully."
                          : "Art not found on GameTDB for GameCube.");
            },
            Qt::QueuedConnection);
      });
...
QVariantMap SwissLibraryService::moveFile(const QString &sourcePath,
...
namespace {
void scanDirectoryRecursivelyGc(SwissLibraryService *self,
...
QVariantList
SwissLibraryService::getExternalGameFilesData(const QStringList &fileUrls) {
  QVariantList files;
